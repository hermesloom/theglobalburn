...
export const BURNER_QUESTIONNAIRE_SPEC = {
  intro: "Borderland is not a show you watch – it’s a magic world we build together. These questions are here to help you get a feel for the beautiful nonsense we are all responsible for. Please take your time when answering them.",
  questions: [
    {
...
export function formatQuestionsAsMarkdown(spec: {
  ...
  for (const q of spec.questions) {
    ...
    if (q.options) {
      const optionType =
        q.type === "radio" ? "Choose one:" : "Choose any that apply:";
      lines.push("*" + optionType + "*");
      lines.push("");
      for (const opt of q.options) {
        lines.push("- " + opt.label);
      }
    } else if (q.type === "number") {
      lines.push("*(Numeric response)*");
    } else {
